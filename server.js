...
async function crawlUnionPos(storeCode, userId, password, targetDate, browser) {
    ...
    try {
        ...
        for (let pageNum = 1; pageNum <= totalPages; pageNum++) {
            ...
            // tableList 테이블의 tbody tr 선택
            const rows = await page.locator('table#tableList tbody tr').all();

            for (const row of rows) {
                try {
                    const cells = await row.locator('td').all();
                    if (cells.length < 11) continue;  // 최소 11개 셀 필요

                    // 첫 번째 셀이 숫자인지 확인 (실제 데이터 행)
                    const firstCell = (await cells[0].textContent()).trim();
                    if (!/^\d+$/.test(firstCell)) continue;

                    // 컬럼 구조: seq, store, sale_datetime, pos_no, receipt_no, total, cash, card, etc, discount, sale_type
        ...
        results.totalSales = totalCash + totalCard + totalEtc;  // 할인은 이미 적용되어 있음
...
async function runPosCrawler(stores = ['chogazip', 'yangeun'], targetDate = null) {
    ...
    try {
        ...
        // 결과 저장 (accounting 데이터에 병합)
        for (const [store, data] of Object.entries(results)) {
            if (data.error) continue;

            const accFile = getAccountingFile(store);
            const accData = readJson(accFile, { dailyRecords: {}, fixedCosts: {} });

            // dailyRecords가 없으면 초기화
            if (!accData.dailyRecords) {
                accData.dailyRecords = {};
            }

            if (!accData.dailyRecords[targetDate]) {
                accData.dailyRecords[targetDate] = {};
            }

            // POS 데이터로 매출 업데이트 (Python 로직과 동일)
            accData.dailyRecords[targetDate].revenue = data.totalSales;
            accData.dailyRecords[targetDate].cashSales = data.cashSales;
            accData.dailyRecords[targetDate].cardSales = data.cardSales;
            accData.dailyRecords[targetDate].etcSales = data.etcSales || 0;
            accData.dailyRecords[targetDate].refundTotal = data.refundTotal || 0;
            accData.dailyRecords[targetDate].voidTotal = data.voidTotal || 0;
            accData.dailyRecords[targetDate].teamCount = data.teamCount;
            accData.dailyRecords[targetDate].crawledAt = new Date().toISOString();
            accData.dailyRecords[targetDate].source = 'unionpos';
...
// POS 수동 실행
app.post('/api/pos/run', async (req, res) => {
    if (posStatus.running) {
